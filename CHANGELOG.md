...
- Added new `option` property `help` to the Custom Fields schema.

## [2.97.0] - 2026-08-21

The ChurchSuite Core API specification has been updated to OpenAPI 3.2, with no functional changes to the API.
...
- Added new `resource_ids[]`, `starts_before`, and `starts_after` filters to `bookings/booked_resources` list endpoint
...
- Added a new POST `giving\donations` endpoint to create a single Donation in the Giving module.

### Fixed

- Fixed the `cheque_number` property on the Donation schema, which had been incorrectly documented as an integer.
